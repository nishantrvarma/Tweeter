...
var followId = document.querySelector("#followId");
...
document.getElementById("followId").disabled = true;
document.getElementById("hashtag").disabled = true;
document.getElementById("mention").disabled = true;

// button press after entering password
loginbutton.addEventListener("click", function (event) {
  console.log("login");
  channel.push("login", { login: loginId.value, passwd: password.value });
  password.value = "";
});

followId.addEventListener("keypress", function (event) {
  if (event.keyCode === 13) {
    channel.push("followPeople", { login: loginId.value, follow: followId.value });
    followId.value = "";
  }
});
...
tweet.addEventListener("keypress", function (event) {
  if (event.keyCode === 13) {
    channel.push("tweet", { login: loginId.value, tweet: tweet.value });
    tweet.value = "";
  }
});

retweet.addEventListener("keypress", function (event) {
  if (event.keyCode === 13) {
    channel.push("retweet", { login: loginId.value, retweet: retweet.value });
    retweet.value = "";
  }
});
...
hashtag.addEventListener("keypress", function (event) {
  if (event.keyCode === 13) {
    channel.push("hashtagFind", { tag: hashtag.value });
    hashtag.value = "";
  }
});

mention.addEventListener("keypress", function (event) {
  if (event.keyCode === 13) {
    channel.push("mentionFind", { tag: mention.value });
    mention.value = "";
  }
});

channel.on("login", function (payload) {
  console.log(payload.listOfTweets[0]);
  var messageItem = document.createElement("li");
  messageItem.innerText = "[" + Date() + "] " + payload.listOfTweets;
  messagesContainer.appendChild(messageItem);
  if (payload.listOfTweets[0] != "Wrong credentials!") {
    document.getElementById("loginId").disabled = true;
    document.getElementById("password").style.display = 'none';
    document.getElementById("passtag").style.display = 'none';
    document.getElementById("loginbutton").style.display = 'none';
    document.getElementById("tweet").disabled = false;
    document.getElementById("retweet").disabled = false;
    document.getElementById("followId").disabled = false;
    document.getElementById("hashtag").disabled = false;
    document.getElementById("mention").disabled = false;
  }
});

channel.on("tweet", function (payload) {
  var messageItem = document.createElement("li");
  messageItem.innerText = "[" + Date() + "]" + " User: " + ("" + payload.id) + " tweeted: " + ("" + payload.tweet);
  messagesContainer.appendChild(messageItem);
});

channel.on("retweet", function (payload) {
  var messageItem = document.createElement("li");
  messageItem.innerText = "[" + Date() + "]" + " User: " + ("" + payload.id) + " retweeted " + ("" + payload.followingId) + "'s tweet : " + ("" + payload.retweet);
  messagesContainer.appendChild(messageItem);
});

channel.on("hashtagFind", function (payload) {
  var messageItem = document.createElement("li");
  messageItem.innerText = "[" + Date() + "] " + payload.tweets;
  messagesContainer.appendChild(messageItem);
});

channel.on("mentionFind", function (payload) {
  var messageItem = document.createElement("li");
  messageItem.innerText = "[" + Date() + "] " + payload.tweets;
  messagesContainer.appendChild(messageItem);
});
